...
- **HospitalCost.com** — *(surfaced 2026-05-27 during TESS pre-screen)* active consumer site, MRF-sourced data for **84 hospitals across Virginia and North Carolina only**, plain-English presentation, layered with CMS quality and safety data. Run by Richard (no public founder bio). Self-described positioning is **identical** to OpenHospitalCost's: "convert complex MRF files into usable consumer information," "every price is a price a hospital published," independent of hospitals/insurers/government, targets high-deductible-plan patients. Explicit expansion intent ("More states are coming") so the regional gap will narrow over time. **Competitive footprint today: small** — 84 hospitals out of ~5,000, 2 states out of 50. **Trademark concern resolved 2026-05-28**: formal TESS search came back clean for "OpenHospitalCost" in Classes 35, 42, and 44, including no registered "HospitalCost" mark surfaced. See [TESS_NOTES.md](TESS_NOTES.md).
...
**TESS cleared (2026-05-28):** formal USPTO search across Classes 35, 42, and 44 came back clean for "OpenHospitalCost." Only adjacent mark surfaced was "Hospital Price Index" (Class 35), a B2B aggregator at hospitalpriceindex.com — compositionally distinct ("Index" vs. "Cost") and operates in a different audience. Coexistence highly defensible. Proceed with branded launch under OpenHospitalCost. Full results in [TESS_NOTES.md](TESS_NOTES.md).

**Trademark filing — decision pending.** Recommended path: DIY Intent-to-Use filing on the wordmark "OpenHospitalCost" in Class 35 only via USPTO TEAS Plus (~$350, ~1 hour). Locks priority date now; Class 42 deferred until the API tier ships. Alternative paths captured in TESS_NOTES.md.
