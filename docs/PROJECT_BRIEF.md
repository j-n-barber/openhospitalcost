...
- **HospitalCost.com** — *(surfaced 2026-05-27 during TESS pre-screen)* active consumer site, MRF-sourced data for **84 hospitals across Virginia and North Carolina only**, plain-English presentation, layered with CMS quality and safety data. Run by Richard (no public founder bio). Self-described positioning is **identical** to OpenHospitalCost's: "convert complex MRF files into usable consumer information," "every price is a price a hospital published," independent of hospitals/insurers/government, targets high-deductible-plan patients. Explicit expansion intent ("More states are coming") so the regional gap will narrow over time. **Competitive footprint today: small** — 84 hospitals out of ~5,000, 2 states out of 50. **Trademark question: separate** — formal TESS search on the "HospitalCost" mark in Classes 35 and 42 still required before launch, because trademark law evaluates mark similarity, not market footprint. "Open" prefix likely provides sufficient distinction (cf. OpenTable, OpenAI), but TESS confirmation needed. See [TESS_NOTES.md](TESS_NOTES.md).
