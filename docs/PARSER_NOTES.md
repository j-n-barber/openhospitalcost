...
- **Approach to bot-blocked sites (Mayo, Memorial Hermann, Boston Children's, etc.):** The `cms-hpt.txt` pattern is universal for compliant hospitals, but the MRF URLs it lists are often gated by Akamai/Imperva/Incapsula. Three options: (a) run downloads through a headless browser (Playwright) for these hospitals only; (b) maintain a static fallback list of known-good URLs scraped from the hospital's price-transparency page via Playwright once, refreshed quarterly; (c) accept these as "deferred coverage" and surface that to the user on hospital pages. Recommendation: (b) for v1, (a) at scale.
