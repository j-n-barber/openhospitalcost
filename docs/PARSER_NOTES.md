...
- **XLSX MRFs — deferred (not a quick win).** Jackson Health's `.xlsx` opens fine with DuckDB `read_xlsx` (the `excel` extension loads in this build), BUT it's a *bespoke* schema: header on row 1, human-readable columns ("Gross Charge", "Payer-Specific Negotiated Charge"), and **MS-DRG** codes (463, 571…), not the CMS template or CPT. Since our dictionary is CPT-only, parsing it yields **zero** matches. XLSX support therefore needs a separate non-template/DRG-schema mapper and only pays off once the procedure dictionary includes DRGs — revisit then.
...
- **403 / bot-blocked** — ✅ Tier-2 wired. `downloadWithFallback` ([fetch/download.js](../pipeline/fetch/download.js)) retries via `getBrowserCookies` ([fetch/browser-fetch.js](../pipeline/fetch/browser-fetch.js)): Playwright clears the JS challenge at the origin, harvests the clearance cookie, then a plain fetch streams the file with that cookie (Playwright's own response buffers in memory — unusable for GB files). The batch counts Tier-2 successes (`stats.viaTier2`).
- **404 / dead URL** — e.g. Norton, Methodist Memphis. Neither tier helps; the `mrf_file_url` is stale and needs re-discovery (re-run the cms-hpt scraper) or Tier-3 manual override.
