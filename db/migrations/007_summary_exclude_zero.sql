-- 007_summary_exclude_zero.sql
-- Exclude non-positive amounts ($0 / negative) from procedure_hospital_summary.
-- Some MRFs carry $0 placeholder rows; with cheapest-first sorting on the money
-- pages those float to the top and read as "free", which is never a real charge.
-- Same view as 006, with `AND p.amount > 0` added to the pr CTE.
...
-- Down Migration
-- DROP MATERIALIZED VIEW IF EXISTS procedure_hospital_summary; (recreate 006 to roll back)
