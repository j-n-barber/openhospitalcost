...
const TIP = {
  negotiated: "Median negotiated facility price across all hospitals with published data, with the low–high range between hospitals.",
  cash: "Median discounted cash (self-pay) price across hospitals.",
  gross: "Median chargemaster list price across hospitals, before any discount.",
};
...
type SortKey = "name" | "hospitals" | "negotiated" | "cash" | "gross";
...
export default function ProcedureIndex({ rows }: { rows: ProcIndexRow[] }) {
  ...
  const view = useMemo(() => {
    ...
    out.sort((a, b) => {
      ...
      if (key === "hospitals") return (a.hospitals - b.hospitals) * dir || a.name.localeCompare(b.name);
      ...
      <table className="ptable">
        <thead>
          <tr>
            ...
            <th className="sortable" style={{ textAlign: "right" }} onClick={() => sortBy("hospitals")}>Hospitals {caret("hospitals")}</th>
