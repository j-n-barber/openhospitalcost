...
export async function GET() {
  const procs = (await sql`
    SELECT DISTINCT p.slug, p.name, p.category
    FROM procedures p
    ...
    ORDER BY p.name`) as { slug: string; name: string; category: string | null }[];
  ...
  const index = [
    ...procs.map((p) => ({ t: "p", nm: titleCaseProcedure(p.name), meta: p.category ?? "Procedure", href: `/procedure/${p.slug}` })),
    ...hospitals.map((h) => ({
      t: "h",
      nm: titleCase(h.name),
